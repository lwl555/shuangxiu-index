...
export default function Policy() {
  return (
    <div className="space-y-8">
      <PageHeader
        kicker="POLICY & VERDICT"
        title="凭什么这么判？"
        desc={
          <>
            很多人以为「双休」就是周六周日一起休，其实法律看的是<strong className="font-semibold text-ink">周均工时</strong>。
      ...
      <section>
        <div className="mb-3 flex items-center gap-2">
          <Icon name="check" className="h-5 w-5 text-rose-600" />
          <h2 className="h-sec">本站判定口径</h2>
        </div>
        <div className="grid gap-4 md:grid-cols-3">
          {[
            { k: '指标一 · 判定主轴', t: '周均工时 ≤ 40 小时', d: '依据《劳动法》第三十六条与国务院令第 174 号。轮休、调休、排班制均按综合计算工时制的周期（周/月/季/年）折算为周均值，超出部分应依法支付加班费。', i: 'clock' },
            { k: '指标二 · 法定底线', t: '每周至少休息 1 天', d: '《劳动法》第三十八条对休息天数的强制要求只是「每周至少休息一日」。休息天数不是否决项——每周休 1.5 天但周均工时 35 小时，法律上完全合规。', i: 'users' },
            { k: '指标三 · 程序', t: '特殊工时须经审批', d: '实行综合计算工时制或不定时工作制须经劳动行政部门审批并告知工会，未经审批的自行安排不予认可。', i: 'award' },
          ].map((x) => (
            <div key={x.t} className="rounded-2xl border border-line bg-white p-5 shadow-soft">
              <div className="kicker">{x.k}</div>
              <div className="mt-2 flex items-center gap-2">
                <Icon name={x.i as any} className="h-4 w-4 text-rose-600" />
                <div className="text-[16px] font-bold text-ink">{x.t}</div>
              </div>
              <p className="mt-2 text-[13px] leading-relaxed text-ink-2">{x.d}</p>
            </div>
          ))}
        </div>
        <div className="mt-4 grid gap-3 md:grid-cols-5">
          {[
            { k: '达标', d: '周均工时 ≤40 小时且每周至少休 1 天', c: '#059669', bg: '#ecfdf5' },
            { k: '基本合规', d: '周均工时 40—44 小时，仍在第三十六条法定上限内', c: '#2563eb', bg: '#eff6ff' },
            { k: '改善中', d: '有明确加班管控，但工时未公开', c: '#d97706', bg: '#fffbeb' },
            { k: '未达标', d: '周均工时超 44 小时，或每周休息不足 1 天', c: '#be123c', bg: '#fff1f2' },
            { k: '数据不足', d: '关键数值缺失，本站不做推测', c: '#4b5563', bg: '#f3f4f6' },
          ].map((x) => (
            <div key={x.k} className="rounded-2xl border border-line bg-white p-4 shadow-soft">
      ...
      <section>
        <div className="mb-3 flex items-center gap-2">
          ...
          <h2 className="h-sec">「轮休等效达标」是什么</h2>
        </div>
        <div className="overflow-hidden rounded-2xl border border-line bg-white shadow-soft">
          <div className="border-b border-line bg-paper-2 p-5">
            <p className="text-[14px] leading-[1.85] text-ink-2">
              ...
              最典型的是<strong className="font-semibold text-ink">建设银行福安支行对私营业网点</strong>
              ——员工每周只休 1~1.5 天，但日均工时压到 6.5 小时，周均工时只有 35.75~39 小时，
              官方回复明确认定「符合国家法律法规规定」。它比很多名义上的双休岗位工时还短。
            </p>
            <p className="mt-2 text-[14px] leading-[1.85] text-ink-2">
              所以本站把判定主轴改成了<strong className="font-semibold text-ink">周均工时</strong>：
              每周休满 2 天记「标准双休」，靠轮休 + 压缩日工时达标者记「轮休等效」，两者同样算达标。
            </p>
          </div>
          <div className="grid gap-4 bg-paper-2 p-5 md:grid-cols-2">
            <div className="rounded-2xl border border-line bg-white p-4 shadow-sm">
              ...
              <div className="mt-2 space-y-1 text-[13px] text-ink-2">
                ...
                <div className="num font-medium">每周休 1—1.5 天 · 日均 6.5 小时 · 周均 35.75—39 小时</div>
              </div>
            ...
            <div className="rounded-2xl border border-line bg-white p-4 shadow-sm">
              ...
              <div className="mt-2 space-y-1 text-[13px] text-ink-2">
                ...
                <div className="num font-medium">8 天周期工作 6 天休 2 天 · 每班 8 小时 · 周均约 42 小时</div>
              </div>
              <p className="mt-2 text-[12.5px] leading-relaxed text-ink-3">
                超过 40 小时但低于《劳动法》第三十六条的 44 小时上限，本站记「基本合规」——这也是绝大多数四班三倒企业的真实位置。
              </p>
            </div>
          </div>
        </div>
      ...
      <section className="rounded-2xl border border-line bg-gradient-to-br from-paper-2 to-white p-5">
        <div className="kicker">轮休的边界</div>
        <h3 className="mt-1.5 text-[16px] font-bold text-ink">不是所有倒班都合规</h3>
        <p className="mt-2 text-[14px] leading-[1.85] text-ink-2">
          轮休合法是有前提的，两个条件缺一不可：<strong className="font-semibold text-ink">经劳动行政部门审批</strong>，
          且<strong className="font-semibold text-ink">周期内平均工时不超过法定标准</strong>。
        </p>
        <p className="mt-2 text-[14px] leading-[1.85] text-ink-2">
      ...
      <section>
        <div className="mb-3 flex items-center gap-2">
          <Icon name="clock" className="h-5 w-5 text-rose-600" />
          <h2 className="h-sec">七种休息模式的定义</h2>
        </div>
        <div className="space-y-3">
          {REST_PATTERNS.map((p) => (
            <div key={p.id} className="flex flex-col gap-4 rounded-2xl border border-line bg-white p-4 shadow-soft md:flex-row md:gap-5">
              <div className="md:w-[190px] md:shrink-0">
                ...
                <div className="mt-1 text-2xs font-medium text-ink-3">{p.short}</div>
                <div className="mt-2">
                  <Tag
                    color={p.compliant === true ? '#059669' : p.compliant === 'partial' ? '#d97706' : '#be123c'}
                    bd={p.compliant === true ? '#a7f3d0' : p.compliant === 'partial' ? '#fde68a' : '#fecdd3'}
                    bg={p.compliant === true ? '#ecfdf5' : p.compliant === 'partial' ? '#fffbeb' : '#fff1f2'}
                  >
              ...
              <div className="flex-1">
                <p className="text-[13.5px] leading-relaxed text-ink-2">{p.desc}</p>
                <p className="mt-2 rounded-lg border-l-2 border-rose-200 bg-rose-50/50 pl-3 text-2xs leading-relaxed text-ink-3">
                  依据：{p.legalBasis}
                </p>
              </div>
            </div>
      ...
      <section>
        <div className="mb-3 flex items-center gap-2">
          <Icon name="award" className="h-5 w-5 text-rose-600" />
          <h2 className="h-sec">法律依据原文</h2>
        </div>
        <div className="space-y-3">
          {LAWS.map((l) => (
            <div key={l.no} className="rounded-2xl border border-line bg-white p-5 shadow-soft">
              <div className="flex flex-wrap items-baseline gap-x-2">
                <span className="num text-[13px] font-bold text-rose-600">{l.no}</span>
                <span className="text-2xs font-semibold text-ink-3">{l.title}</span>
              </div>
              <p className="mt-2 text-[14px] leading-[1.75] text-ink-2">{l.text}</p>
              {l.note && (
                <p className="mt-3 rounded-lg border-l-2 border-rose-200 bg-rose-50/50 py-2 pl-3 text-[12.5px] leading-relaxed text-ink-3">
                  {l.note}
                </p>
              )}
            </div>
      ...
      <section>
        <div className="mb-3 flex items-center gap-2">
          <Icon name="info" className="h-5 w-5 text-rose-600" />
          <h2 className="h-sec">常见误区</h2>
        </div>
        <div className="space-y-3">
          {FAQ.map((f, i) => (
            <details key={i} className="group overflow-hidden rounded-2xl border border-line bg-white shadow-soft">
              <summary className="flex cursor-pointer list-none items-center justify-between px-5 py-4 text-[14px] font-semibold text-ink transition-colors hover:bg-rose-50/30">
                <span>
                  <span className="num mr-2 text-rose-600">{String(i + 1).padStart(2, '0')}</span>
                  {f.q}
                </span>
                <span className="ml-3 text-ink-3 transition-transform group-open:rotate-45">
                  <Icon name="zap" className="h-4 w-4" />
                </span>
              </summary>
              <p className="border-t border-line px-5 py-4 pl-[3.2rem] text-[13.5px] leading-[1.8] text-ink-2">{f.a}</p>
            </details>
          ))}
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-violet-100 bg-gradient-to-br from-violet-50 to-white p-5">
        <div className="kicker text-violet-600">外部变量</div>
        <h3 className="mt-1.5 text-[16px] font-bold text-ink">《欧盟市场禁止强迫劳动产品条例》</h3>
        <p className="mt-2 text-[14px] leading-[1.85] text-ink-2">
          该条例将于 2027 年底实施，明确出口欧盟产品的任何环节均不得涉及强迫劳动，
          <strong className="font-semibold text-ink">每周超过 40 小时的加班即便员工自愿也在禁止之列</strong>。
